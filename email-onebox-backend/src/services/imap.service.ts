...
import { indexEmail, getAddressText } from './elasticsearch.service';
...
const processEmail = async (parsedEmail: ParsedMail, config: Imap.Config) => {
    ...
    const category = await categorizeEmail(parsedEmail.subject || '', parsedEmail.text || '');
    ...
    if (category === 'Interested') {
        ...
        await sendSlackNotification(parsedEmail.subject || 'No Subject', fromAddress);
