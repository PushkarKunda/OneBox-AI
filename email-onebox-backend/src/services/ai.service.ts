 ...
 */
export const categorizeEmail = async (subject: string, body: string): Promise<EmailCategory> => {
    try {
        // Combine subject and body for better context
        const inputText = `Subject: ${subject}\n\nBody: ${body}`;
        
        // These are the categories the model will choose from
        const candidateLabels: EmailCategory[] = ['Interested', 'Meeting Booked', 'Not Interested', 'Spam', 'Out of Office'];
        ...
        if (response && response.length > 0) {
            ...
            if (topCategory.score > 0.7) { 
                console.log(`HF Classified as "${topCategory.label}" with score ${topCategory.score.toFixed(2)}`);
                return topCategory.label as EmailCategory;
            }
        }

        console.log('HF classification confidence was too low, defaulting to Uncategorized.');
